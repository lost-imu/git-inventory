...
namespace InventorySystem1._0
{
    ...
    public partial class frmStockOut : Form
    {
        ...
        private void FrmStockOut_Load(object sender, EventArgs e)
        {
            ...
            sql = "SELECT `ITEMID` as 'رقم الصنف', `NAME` as 'الاسم', `DESCRIPTION` as 'الشرح', `TYPE` as 'النوع', `QTY` as 'الكمية المتوفرة', `UNIT` as 'الوحدة', `PROJECT` as 'المشروع', case when ISNEW =0 then 'مستعمل' else 'جديد' END as 'جديد-\nمستعمل', case when `EXPIRYDATE` IS NULL then '' ELSE `EXPIRYDATE` END as 'انتهاء الصلاحية', `PROJECTEXPIRY` as 'نهاية المشروع', `NOTE` as 'ملاحظات' FROM `tblitems`";
